...
class ObjectsObstacle:
    def __init__(self, obstacles=1):
        ...
        self.wallId = self.p.createCollisionShape(self.p.GEOM_BOX,
                                                  halfExtents=[wallHalfLength, wallHalfWidth, wallHalfHeight])

        self.wall1, self.wall2, self.wall3 = self.spawn_walls()
    ...
    def spawn_walls(self):
        wall1, wall2, wall3 = None, None, None
        wall_ori1 = self.p.getQuaternionFromEuler([0, math.pi / 2, math.pi / 2])
        wall_ori2 = self.p.getQuaternionFromEuler([0, 0, -math.pi / 4])
        wall_ori3 = self.p.getQuaternionFromEuler([0, 0, math.pi / 4])

        if self.obstacles == 1:
            wall1 = self.p.createMultiBody(baseMass=0,
                                           baseCollisionShapeIndex=self.wallId,
                                           basePosition=[1, 0, 0.2], baseOrientation=wall_ori1)
            self.p.changeDynamics(wall1, -1, restitution=0.7)

        else:
            wall2 = self.p.createMultiBody(baseMass=0,
                                           baseCollisionShapeIndex=self.wallId,
                                           basePosition=[1, 0.15, 0.2], baseOrientation=wall_ori2)
            wall3 = self.p.createMultiBody(baseMass=0,
                                           baseCollisionShapeIndex=self.wallId,
                                           basePosition=[1, -0.15, 0.2], baseOrientation=wall_ori3)

            self.p.changeDynamics(wall2, -1, restitution=0.7)
            self.p.changeDynamics(wall3, -1, restitution=0.7)
    ...
    def change_ori(self, wall_id, wall_pos, random_ori):
        target_ori = self.p.getQuaternionFromEuler([0, 0, random_ori])
        self.p.resetBasePositionAndOrientation(wall_id, wall_pos, target_ori)
    ...
    def step(self, action):
        self.push_degree(degree=action)
    ...
    def episode(self, action, wall_ori=0):

        # for action in action_list:
        if self.obstacles == 1:
            self.change_ori(self.wall1, [1, 0, 0.2], wall_ori)
        self.step(action)

        # rgb_array = self.render()

        # return self.get_robot_pos_ori(self.link), self.get_base_pos_ori(self.duck)
